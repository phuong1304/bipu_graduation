...
import { loginUser, findUserByUsername } from "@/app/actions/auth";
import { upsertParticipant } from "@/app/actions/participants";
...
export default function ParticipantLoginPage() {
    ...
    const startExperienceTransition = async (username: string) => {
        setIsTransitioning(true);

        // Wait for animation
        await new Promise((resolve) => setTimeout(resolve, TRANSITION_DELAY_MS));

        // Then login and refresh
        await loginUser(username, "user");
        startTransition(() => {
            router.refresh();
        });
    };
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            if (existingUser) {
                await startExperienceTransition(normalizedUsername);
            } else {
            ...
            setError(message);
            setIsLoading(false); // Only stop loading on error or modal open
        } finally {
            if (!isTransitioning) {
                setIsLoading(false);
            }
    ...
    const handleProfileSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            await startExperienceTransition(pendingUsername);
        } catch (err) {
            const message =
                        ...
                        <div className="bg-white/95 backdrop-blur-lg rounded-[2.65rem] grid gap-0 lg:grid-cols-2 overflow-hidden">
                            <section className="p-8 sm:p-10 lg:p-12 border-b border-gray-100/70 lg:border-b-0 lg:border-r space-y-4">
                                ...
                                <div className="rounded-[16px] border border-indigo-100 overflow-hidden shadow-2xl">
                                    <img
                                        src="/assets/icon/login-image.jpg"
