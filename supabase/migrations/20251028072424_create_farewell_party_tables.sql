...
CREATE POLICY "Allow anonymous users to insert RSVP"
  ON rsvp_responses
  FOR INSERT
  TO anon
  WITH CHECK (true);

CREATE POLICY "Allow anonymous users to read RSVP"
  ON rsvp_responses
  FOR SELECT
  TO anon
  USING (true);
...
CREATE POLICY "Allow anonymous users to insert wishes"
  ON wishes
  FOR INSERT
  TO anon
  WITH CHECK (true);

CREATE POLICY "Allow anyone to read wishes"
  ON wishes
  FOR SELECT
  TO anon
  USING (true);
