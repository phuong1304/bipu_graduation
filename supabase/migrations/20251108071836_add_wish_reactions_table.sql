...
CREATE TABLE IF NOT EXISTS wish_reactions (
    ...
    reactor_name text NOT NULL DEFAULT 'Guest',
    created_at timestamptz DEFAULT now()
);

ALTER TABLE wish_reactions ENABLE ROW LEVEL SECURITY;

<< << << < Updated upstream CREATE POLICY "Allow anonymous users to insert reactions" ON wish_reactions FOR
INSERT
    TO anon
WITH
    CHECK (true);

CREATE POLICY "Allow anyone to read reactions" ON wish_reactions FOR
SELECT TO anon USING (true);

CREATE INDEX IF NOT EXISTS idx_wish_reactions_wish_id ON wish_reactions (wish_id);

CREATE UNIQUE INDEX IF NOT EXISTS idx_wish_reactions_unique_session_sticker ON wish_reactions (wish_id, session_id, sticker);

= = = = = = = CREATE POLICY "Allow anonymous users to insert reactions" ON wish_reactions FOR
INSERT
    TO anon
WITH
    CHECK (true);
